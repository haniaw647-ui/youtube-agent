...
async def run(job_id: str, tenant_id: str) -> dict:
    ...
    voice = (
        ElevenLabsProvider(api_key) if provider_name == "elevenlabs" else OpenAITTSProvider(api_key)
    )
    ...
    async with service_session() as session:
        await session.execute(
            text(
                "INSERT INTO assets (tenant_id, job_id, type, storage_path, provider) "
                "VALUES (:tenant_id, :job_id, 'voice', :storage_path, :provider)"
            ),
            {
    ...
    return {"asset_type": "voice", "provider": provider_name, "storage_path": storage_path}
